...
| The hiding rule survives the cascade | `getComputedStyle` read on a live published page | Of the 28 sidebar items, the 14 in the other language computed to `display: none`. The rule is not overridden by anything the theme loads after it, which no markup-level check can establish |
| Local preview | `npm run serve`, then requests against `http://127.0.0.1:4000/github-copilot-app-workshop/` | Both language home pages, a deep facilitator page, and a `.pptx` download returned HTTP 200. The toggle round-tripped English to French and back, leaving 14 links of one language visible each time |
| Runner toolchain matches the pins | Setup Node and Setup Ruby step logs from the deploy run | The `ubuntu-24.04` runner installed `node v26.7.0` and `ruby 3.2.11`, the two versions the workflow pins. The pins are reachable on the hosted image rather than assumed to be |
| Both decks open in PowerPoint and render | PowerPoint 16.0 through COM automation, exporting every slide to PNG at 1600 by 900 | Both files opened, each reporting 16 slides on a 16:9 stage. All 32 slides exported as non-empty images. See the human-judged section for what was then looked at |
...
* The five highest-risk slides — French slides 1, 7, 12, and 14, and English slide 3, which carry the longest footer, the longest bodies, the longest kicker, and the longest titles — were examined as text extracted from the binaries and read directly, with each string measured against its box. The strings fit, and the accented characters survived generation intact.
* All 32 slides have since been rendered to images by PowerPoint, and three of them were looked at: the English opening slide, the English closing slide, and a mid-deck French slide carrying `é`, `è`, and `À`. Type, rules, and footers sat where the generator places them, nothing overflowed or clipped, and the French diacritics rendered as letters rather than as substitution boxes. Those three slides also show that each one fills roughly its top third and leaves the rest empty, which is a pacing and design question for the curriculum owner rather than a defect. **Twenty-nine slides were rendered but not looked at, and no design review has happened.** See DR-08 below.
...
* **The visual appearance of 29 of the 32 slides.** Every slide has been rendered, but only three were examined by eye. Font substitution, contrast, and visual balance across the remaining 29 are unexamined, and no design review has been performed on any of them. This is blocking gate DR-08.
...
* **Sidebar behaviour in a browser without CSS `:has()`.** Both degraded paths that can be reproduced here were exercised on a live page. With JavaScript disabled the rule alone scoped the sidebar and no link of the other language was visible; with the `:has()` rule deleted at runtime the JavaScript fallback alone hid the same 14 items. Neither result tells you how a browser that genuinely lacks `:has()` behaves, because no such browser is available here and the current engine cannot pretend to be one. Where neither mechanism is available both language trees stay visible, which is a cosmetic regression rather than a broken site, and that path has not been observed.
...
| DR-08 | All 32 slides have now been rendered and three were inspected. The other 29 have not been looked at, and the design review by the curriculum owner has not happened. | Curriculum lead | The remaining 29 slides inspected, plus a design review by the curriculum owner |
...
DR-08 deserves a plain statement, because an earlier revision of this page claimed it had been closed when no renderer existed at all. A renderer has since been found: PowerPoint 16.0 is installed here, and both decks now open in it and export every slide to an image. That removes the part of the gate that was about capability. What remains is judgment. Three slides were looked at and the other 29 were not, and nobody with curriculum ownership has reviewed any of them. **Rendering 32 images is not the same as reviewing a deck.** The gate stays open, narrower than it was.
