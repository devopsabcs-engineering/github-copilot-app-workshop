...
Recorded 2026-09-22, at the end of authoring. This page exists so that nobody has to infer the state of this workshop from the fact that a build passed.
...
| Accessibility, every built page | axe-core 4.13.0, WCAG 2.0/2.1/2.2 A and AA rulesets, all 28 pages at 1280 and at 400 CSS pixels | Zero violations across all 56 runs, 16,101 passing checks |
...
| Reflow | All 28 pages at 640 and at 320 CSS pixels | No horizontal scrolling in any of the 56 checks |
...
* **Screen reader announcement.** The navigation was confirmed to mark every link with its own language: 28 links per page, 14 declaring `en` and 14 declaring `fr`, with the rendered sidebar byte-identical across both language trees, exactly as the cached-sidebar constraint requires. **That is markup, not announcement.** No screen reader was run. Whether any assistive technology actually switches voice on those attributes is unknown, so WCAG 3.1.2 conformance for the navigation is unconfirmed. Emitting a `lang` attribute is not evidence that anything is announced differently.
* **Colour contrast in the site title and the sidebar links.** axe-core returned these as *incomplete*, not as passes: it could not compute them because the elements are partially obscured or sit on a gradient. There were 75 such nodes across the 28 pages at 1280 CSS pixels, between two and five per page depending on how many sidebar links that page renders, and none at 400 pixels where the sidebar collapses. Recomputing the ratios by hand from the rendered colours puts all of them above AA, the thinnest being the sidebar link at 5.03:1 against a 4.5:1 requirement. That margin is thin enough that a theme change could cross it silently, and a hand calculation is not the same as a tool pass.
...
* **The published site.** GitHub Pages is not enabled, so the workflow has never run and no page has ever been served from the real base path. Everything above was verified against a local build.
...
| DR-09 | GitHub Pages is not enabled on this repository. The workflow's configure step fails on its first run, including on pull requests. | Repository owner | Pages enabled with the Actions source, and one green workflow run |
| DR-10 | The `ruby/setup-ruby` action is absent from the repository Actions allow-list. The build job cannot provision Ruby. | Repository or organization owner | The action present in the allow-list, and one green workflow run |
...
Re-run the checks and re-date this page when the content model changes, when the theme or any dependency is upgraded, when a rehearsal is performed, or when Pages is enabled. A validation record that outlives the thing it describes is worse than none, because it invites exactly the assumption this page exists to prevent.
