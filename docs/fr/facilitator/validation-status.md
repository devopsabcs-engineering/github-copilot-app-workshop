...
| Accessibilité, toutes les pages publiées | axe-core 4.13.0, règles WCAG 2.0/2.1/2.2 niveaux A et AA, les 28 pages à 1280 puis à 400 pixels CSS, demandées sur le site en ligne | Aucune violation sur les 56 passages, 15 677 vérifications réussies |
...
| Redistribution du contenu | Les 28 pages publiées à 640 puis à 320 pixels CSS | Aucun défilement horizontal sur les 56 vérifications |
...
| Navigation restreinte à une seule langue | `npm run validate:content`, plus un vrai navigateur à 1280 pixels CSS | Chacune des 28 pages construites porte dans son en-tête une règle masquant l'autre langue, et aucune ne masque la sienne ; les 28 liens de la barre latérale déclarent tous une langue, si bien que chaque page en laisse exactement 14 visibles. Dans le navigateur, une page anglaise affichait six liens anglais de premier niveau et aucun lien français, une page française l'inverse |
| La règle de masquage l'emporte dans la cascade | Lecture de `getComputedStyle` sur une page publiée en direct | Sur les 28 éléments de la barre latérale, les 14 de l'autre langue calculaient `display: none`. La règle n'est donc écrasée par rien de ce que le thème charge ensuite, ce qu'aucune vérification du balisage ne peut établir |
| Aperçu local | `npm run serve`, puis des requêtes vers `http://127.0.0.1:4000/github-copilot-app-workshop/` | Les pages d'accueil des deux langues, une page d'animation profonde et un téléchargement `.pptx` ont répondu HTTP 200. Le sélecteur a fait l'aller-retour de l'anglais au français puis retour, en laissant chaque fois 14 liens d'une seule langue visibles |
| La chaîne d'outils du coureur correspond aux versions figées | Journaux des étapes Setup Node et Setup Ruby de l'exécution de déploiement | Le coureur `ubuntu-24.04` a installé `node v26.7.0` et `ruby 3.2.11`, les deux versions figées par le flux. Ces versions sont donc réellement disponibles sur l'image hébergée, et non simplement supposées |
| Les deux supports s'ouvrent dans PowerPoint et se rendent | PowerPoint 16.0 piloté par COM, chaque diapositive exportée en PNG à 1600 sur 900 | Les deux fichiers se sont ouverts, chacun annonçant 16 diapositives sur une scène 16:9. Les 32 diapositives ont été exportées en images non vides. Voir la section de jugement humain pour ce qui a ensuite été regardé |
...
* Les cinq diapositives les plus à risque — les françaises 1, 7, 12 et 14, et l'anglaise 3, celles qui portent le pied de page le plus long, les corps de texte les plus longs, l'accroche la plus longue et les titres les plus longs — ont été examinées sous forme de texte extrait des binaires et lu directement, chaque chaîne étant mesurée par rapport à son cadre. Les chaînes tiennent, et les caractères accentués ont survécu à la génération.
* Les 32 diapositives ont depuis été rendues en images par PowerPoint, et trois ont été regardées : la diapositive d'ouverture anglaise, la diapositive de clôture anglaise, et une diapositive française du milieu portant `é`, `è` et `À`. La typographie, les filets et les pieds de page se trouvaient là où le générateur les place, rien ne débordait ni n'était tronqué, et les signes diacritiques français se sont affichés comme des lettres et non comme des carrés de substitution. Ces trois diapositives montrent aussi que chacune occupe environ son tiers supérieur et laisse le reste vide, ce qui relève du rythme et de la conception, donc du responsable pédagogique, et non d'un défaut. **Vingt-neuf diapositives ont été rendues sans être regardées, et aucune revue de conception n'a eu lieu.** Voir DR-08 plus bas.
...
* **Contraste du titre du site et des liens de la barre latérale.** axe-core les a renvoyés comme *indéterminés*, et non comme réussis : l'outil n'a pas pu les calculer, ces éléments étant partiellement masqués ou posés sur un dégradé. On en compte 56 sur les 28 pages à 1280 pixels CSS, et aucun à 400 pixels où la barre latérale se replie. Le recalcul manuel à partir des couleurs réellement rendues les place tous au-dessus du niveau AA, le plus mince étant le lien de la barre latérale à 5,03:1 pour une exigence de 4,5:1. Cette marge est assez mince pour qu'une évolution du thème la franchisse sans bruit, et un calcul manuel n'équivaut pas à une réussite outillée.
* **L'aspect visuel de 29 des 32 diapositives.** Toutes ont été rendues, mais trois seulement ont été examinées à l'œil. La substitution de police, le contraste et l'équilibre visuel des 29 autres restent inexaminés, et aucune revue de conception n'a porté sur l'une d'entre elles. C'est l'exigence bloquante DR-08.
...
* **Le comportement de la barre latérale dans un navigateur sans `:has()`.** Les deux modes dégradés reproductibles ici ont été exercés sur une page en direct. JavaScript désactivé, la règle seule a restreint la barre latérale et aucun lien de l'autre langue n'était visible ; la règle `:has()` supprimée à l'exécution, la solution de repli en JavaScript a masqué à elle seule les mêmes 14 éléments. Aucun de ces résultats ne dit comment se comporte un navigateur qui ne connaît réellement pas `:has()`, car aucun n'est disponible ici et le moteur actuel ne peut pas feindre de l'ignorer. Là où ni l'une ni l'autre n'est disponible, les deux arborescences restent visibles, ce qui est une régression esthétique et non un site cassé, et ce mode n'a pas été observé.
...
| DR-08 | Les 32 diapositives ont maintenant été rendues et trois ont été inspectées. Les 29 autres n'ont pas été regardées, et la revue de conception par le responsable pédagogique n'a pas eu lieu. | Responsable pédagogique | Les 29 diapositives restantes inspectées, puis une revue de conception par le responsable pédagogique |
...
DR-08 mérite d'être dit clairement, parce qu'une version précédente de cette page la déclarait refermée alors qu'aucun outil de rendu n'existait. Un outil a depuis été trouvé : PowerPoint 16.0 est installé ici, les deux supports s'y ouvrent et exportent chaque diapositive en image. Cela retire de l'exigence la part qui tenait à la capacité. Ce qui reste relève du jugement. Trois diapositives ont été regardées, les 29 autres non, et personne ayant la responsabilité pédagogique n'en a revu une seule. **Produire 32 images n'équivaut pas à relire un support.** L'exigence reste ouverte, plus étroite qu'avant.
